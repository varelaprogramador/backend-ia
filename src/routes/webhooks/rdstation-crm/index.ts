...
const dealDocumentSchema = z.object({
  ...
  status: z.enum(["ongoing", "won", "lost"]).optional().nullable(),
...
async function handleDealCreated(payload: z.infer<typeof webhookPayloadSchema>) {
  const document = dealDocumentSchema.parse(payload.document);

  if (!document.deal_pipeline?.id) {
    logWarn("Deal created without pipeline_id, skipping", { dealId: document.id });
    return;
  }

  // Buscar funil vinculado ao pipeline do RD Station
  const funnel = await db.funnel.findFirst({
    where: { rdstationPipelineId: document.deal_pipeline.id },
    include: { stages: { orderBy: { order: "asc" } } },
  });

  if (!funnel) {
    logInfo("No funnel linked to RD Station pipeline", {
      pipelineId: document.deal_pipeline.id,
...
async function handleDealUpdated(payload: z.infer<typeof webhookPayloadSchema>) {
  const document = dealDocumentSchema.parse(payload.document);
  ...
  if (!lead) {
    logInfo("No lead found for deal update, attempting to create", { dealId: document.id });
...
async function handleDealDeleted(payload: z.infer<typeof webhookPayloadSchema>) {
  ...
  if (!lead) {
    logInfo("No lead found for deleted deal", { dealId: document.id });
    return;
  }
...
async function handleLostReasonEvent(payload: z.infer<typeof webhookPayloadSchema>) {
    ...
    try {
      ...
      switch (payload.event_name) {
        ...
        case "crm_lost_reason_deleted":
          await handleLostReasonEvent(payload);
          break;

        default:
          logWarn("Unknown RD Station webhook event", { event: payload.event_name });
      }

      return formatResponse({
        message: "Webhook processed successfully",
        data: { transactionId: payload.transaction_uuid },
      });
    } catch (error: any) {
